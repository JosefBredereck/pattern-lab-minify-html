...
module.exports = function(patternlab, pattern) {
  ...
  if (!patternlab) {
    console.error(pluginName + ': patternlab object not provided to findTab');
    process.exit(1);
  }

  if (!pattern) {
    console.error(pluginName + ': pattern object not provided to findTab');
    process.exit(1);
  }

  let options = patternlab.config.plugins[pluginName].options;
  let pluginOptions = patternlab.config.plugins[pluginName].pluginOptions;
